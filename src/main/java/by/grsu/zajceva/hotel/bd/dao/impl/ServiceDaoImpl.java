...
public class ServiceDaoImpl extends AbstractDao implements IDao<Integer, Service> {
	...
	@Override
	public List<Service> find(TableStateDto tableStateDto) {
		List<Service> entitiesList = new ArrayList<>();
		try (Connection c = createConnection()) {
			...
			System.out.println("searching Services using SQL: " + sql);
	...
	public int count() {
		try (Connection c = createConnection()) {
			PreparedStatement pstmt = c.prepareStatement("select count(*) as c from service");
			ResultSet rs = pstmt.executeQuery();
			rs.next();
			return rs.getInt("c");
		} catch (SQLException e) {
			throw new RuntimeException("can't get services count", e);
		}
	}
}